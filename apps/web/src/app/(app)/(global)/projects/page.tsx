...
function ProjectsPageContent() {
  ...
  const { data: projects, isLoading, error } = useProjects(activeOrgId || undefined);
    ...
    <div className="w-full flex flex-col h-full p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 mb-4 border-b border-[var(--border-subtle)] shrink-0">
        <div className="min-w-0 flex-1">
          {activeOrg && (
            <div className="flex items-center gap-1.5 text-[12px] font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-1">
          ...
          <p className="mt-1 text-[14px] text-[var(--text-secondary)]">
            {activeOrg
              ? `Projects belonging to ${activeOrg.name}.`
      ...
      {!isLoading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 flex-1 content-start">
          {projects?.map((project) => {
            const projectHref = activeOrgId
              ? `/orgs/${activeOrgId}/projects/${project.id}`
              : `/projects/${project.id}`;

            return (
              <Link href={projectHref} key={project.id}>
      ...
      <CreateProjectDialog
        isOpen={showModal}
        onClose={closeModal}
        defaultOrganizationId={activeOrgId || undefined}
      />
    </div>
  );
}
