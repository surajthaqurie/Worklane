...
export function AppSidebar({ isCollapsed, onToggle }: AppSidebarProps) {
  ...
  // Extract projectId from pathname since useParams() in parent layout is empty
  ...
  return (
    ...
    >
      <div className="flex items-center h-14 px-3 border-b border-[var(--border-subtle)] cursor-pointer hover:bg-[var(--bg-surface-hover)] transition-colors" onClick={() => !isCollapsed && setIsDropdownOpen(!isDropdownOpen)}>
        <div className="flex items-center text-[var(--text-primary)] font-semibold w-full justify-between">
          <div className="flex items-center">
            {currentProject ? (
               <div className={`w-6 h-6 rounded-[var(--radius-button)] bg-[var(--bg-surface-selected)] border border-[var(--border-subtle)] flex items-center justify-center text-[10px] font-bold text-[var(--brand-primary)] ${isCollapsed ? 'mx-auto' : 'mr-2'}`}>
      ...
      {!isCollapsed && isDropdownOpen && (
        <>
          ...
          <div className="absolute top-14 left-2 right-2 bg-[var(--bg-surface-elevated)] border border-[var(--border-subtle)] rounded-[var(--radius-card)] shadow-lg z-50 py-2 flex flex-col max-h-[300px]">
            <div className="px-3 pb-2 text-[11px] font-bold text-[var(--text-muted)] uppercase tracking-wider">Projects</div>
