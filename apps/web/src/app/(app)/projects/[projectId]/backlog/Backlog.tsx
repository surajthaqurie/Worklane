...
export function Backlog({ projectId }: { projectId: string }) {
  ...
  const renderRow = (item: WorkItem, depth: number) => {
    ...
    return (
      <React.Fragment key={item.id}>
        <div 
          className="grid grid-cols-[auto_100px_1fr_100px_100px_100px_120px_120px_100px] gap-4 py-2 px-4 border-b border-[var(--border-subtle)] bg-[var(--bg-surface)] hover:bg-[var(--bg-surface-hover)] transition-colors items-center cursor-pointer group"
          onClick={() => {
        ...
        >
          <div 
            ...
            style={{ width: `${depth * 20 + 24}px`, paddingLeft: `${depth * 20}px` }}
        ...
        {creatingParentId === item.id && (
          <div className="flex items-center gap-3 py-2 px-4 border-b border-[var(--border-subtle)] bg-[var(--bg-surface-selected)]">
            <div style={{ width: `${(depth + 1) * 20 + 24}px` }} />
            <select
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-[var(--radius-card)] overflow-hidden flex-1 flex flex-col mb-4">
